...
int main (int argc, char **argv) 
{
	...
	vor::Vertices::iterator i = ver->begin(); 
	  (*i)->x = 1000;
	  (*i)->y = 1000;
	  ++i;
	  (*i)->x = 1100;
	...
	test1(n,2); //function to generate centroids
...
{	
	 ...
	 * */
	  ...
	  for(int z = -250;z <250;z++)
		{		 
		  ...
		  {
			  int f = (*i)->y-k;
			  if(pow(((*i)->x - e), 2) + pow(((*i)->y - f), 2) <= 125000)
			  {
				  p[e][f] = 1;
			  }
		  }
	...
	for(vor::Vertices::iterator i = ver->begin(); i!= ver->end(); ++i)
	{
		...
		{	
			...
			float a = r * cosf(theta);//calculate the x component
			float b = r * sinf(theta);//calculate the y component

			glVertex2f(-1+2*(a + (*i)->x/w),-1+2*(b + (*i)->y/w));//output vertex
	  ...
	  for(int i= 0;i<10000;i++)
	  {
		  for(int j= 0; j<10000;j++)
		  {
			...
			if(p[i][j]==1)
			{
				glBegin(GL_QUADS);
				glColor3f(0,0,1);
				glVertex2f( -1+2*i/w -0.01,  -1+2*j/w - 0.01);
				glVertex2f( -1+2*i/w +0.01,  -1+2*j/w - 0.01);
				glVertex2f( -1+2*i/w +0.01,  -1+2*j/w + 0.01);
				glVertex2f( -1+2*i/w -0.01,  -1+2*j/w + 0.01);
